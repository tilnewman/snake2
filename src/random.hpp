...
namespace util
{
    ...
    class Random
    {
      public:
        Random()
            : m_warmupSkipCount(123456) // see comment below
        ...
        T fromTo(const T from, const T to) const
        {
            ...
            {
                // note how nextafter() is required here
      ...
      private:
        // the first few numbers generated by mt19937 are easily predictable if the seed is bad,
        // so skip them. warmup is also just a good practice when working with PRNGs anything from
